...
TEST(DiscreteFourierTransformTest, DFTTest) {
  ...
  std::vector<std::complex<double>> expected_y = {
    ...
    std::complex<double>(-4, 9.6568542495),
    std::complex<double>(-4, 4),
    std::complex<double>(-4, 1.6568542495),
  ...
  for (int i = 0; i < y.size(); i++) {
    EXPECT_NEAR(y[i].real(), expected_y[i].real(), 1e-10);
    EXPECT_NEAR(y[i].imag(), expected_y[i].imag(), 1e-10);
  }
}
...
TEST(DiscreteFourierTransformTest, IDFTTest) {
  ...
  for (int i = 0; i < x.size(); i++) {
    EXPECT_NEAR(x[i].real(), expected_x[i].real(), 1e-10);
    EXPECT_NEAR(x[i].imag(), expected_x[i].imag(), 1e-10);
  }
}
